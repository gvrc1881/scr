...
@Component({
    ...
    templateUrl: './approve-psi-thermovision-measures.component.html',
    styleUrls: ['./approve-psi-thermovision-measures.component.css'],
    providers: [
    {
...
export class ApprovePsiThermovisionMeasuresComponent implements OnInit {
  ...
  Titles = FieldLabelsConstant.TITLE;
    ...
    displayedColumns =['sno','station','point1description','measure1','point2description','measure2','tempDiff','remark','check']//,'fixed','criticality','previous1','previous2','previous3','actions'
    ...
    getThermoMeasuresData(){
        ...
        this.sendAndRequestService.requestForGET(Constants.app_urls.THERMOVISION.THERMOVISION_MEASURE.GET_NON_APPROVED_PSI_THERMO_MEASURES
    ...
    getDepots(){
        ...
        this.depotsList = this.depotsData.filter(value => {
            if(this.inputFormGroup.value.depotType) {
              var depotType = this.inputFormGroup.value.depotType ;
              return value.division.toLowerCase() == division.toLowerCase() &&  value.depotType == depotType;  
            }else {
                if(value.depotType == "TSS" || value.depotType == "SSP" || value.depotType == "SP"){
