...
CREATE OR REPLACE VIEW public.v_energy_consumption AS 
 SELECT ec.id, ec.seq_id,
    ...
    em.em_start_date AS em_start_date,
    em.em_end_date AS em_end_date,
    em_end_kwh , em_end_kvah , em_end_rkvah_lag ,em_end_rkvah_lead ,	
    em_start_kwh , em_start_kvah , em_start_rkvah_lag ,	em_start_rkvah_lead ,
     multiplication_fac ,
    em.remarks AS em_remarks
   FROM energy_consumption ec, v_energy_meter em

  WHERE  ec.location = em.feeder_name 
  AND ec.energy_reading_date >= em_start_date 
  AND (ec.energy_reading_date <= em_end_date OR em_end_date IS NULL);
