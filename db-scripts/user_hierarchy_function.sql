-- Function: user_func_location(text)

-- DROP FUNCTION user_func_location(text);

CREATE OR REPLACE FUNCTION user_func_location(userid text)
  RETURNS text AS
$BODY$
...
DECLARE
        ...
        UNIT_NAME TEXT;
               ...
               CUR_UNIT_NAMES CURSOR FOR SELECT UNIT_CODE FROM FUNCTIONAL_LOCATION_HIERARCHY flh_l where flh_l.head_login_id not in (select distinct flh_r.rm_login_id from FUNCTIONAL_LOCATION_HIERARCHY flh_r) order BY ID;
...
BEGIN
               ...
               IF  NO_OF_REPORTEES = 0 THEN  --USER HAS NO REPORTEES THEN HIS DEFAULT DEPOT/WH IS ONLY ONE UNIT
                       ...
                       SELECT FACILITY_NAME 
                              ...
                              WHERE FACILITY_TYPE_ID in ('WAREHOUSE', 'PLANT')
               ...
               RAISE NOTICE ' UNIT NAME------- For RM   % -',USERID ;
                              ...
                              SELECT ID INTO user_id FROM FUNCTIONAL_LOCATION_HIERARCHY WHERE head_LOGIN_ID = USERID;

                              RAISE NOTICE 'user name id is ------- -% --', user_id;

                              UNIT_LIST := array(

                              WITH RECURSIVE nodes(id,unit_code, unit_type, head_login_id, rm_seq_id ) AS

                              (

                              SELECT f1.id, f1.unit_code,  f1.unit_type, f1.head_login_id, f1.rm_seq_id

                              FROM functional_location_hierarchy f1 WHERE f1.rm_seq_id::integer =  user_id::integer

                              UNION

                              SELECT f2.id, f2.unit_code,  f2.unit_type, f2.head_login_id, f2.rm_seq_id

                              FROM functional_location_hierarchy f2, nodes f1 WHERE f2.rm_seq_id::integer = f1.id::integer
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION user_func_location(text)
  OWNER TO postgres;
